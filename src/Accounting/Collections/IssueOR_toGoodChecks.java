...
public class IssueOR_toGoodChecks extends _JInternalFrame implements _GUI, ActionListener, MouseListener {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3061284418918863916L;
	static String title = "Issuance of OR for Good Checks";
	...
	/*	Additional Controls by Mann2x; Date Added: August 1, 2017;	*/
	...
	// ADDED BY JARI CRUZ DTD MAY 10 2023 DCRF# 2581
	private JComboBox cmbIssuanceType;
	private String strType = "OR";

	public IssueOR_toGoodChecks() {
		super(title, true, true, true, true);
		initGUI();
	}

	public IssueOR_toGoodChecks(String title) {
		super(title);

	}

	public IssueOR_toGoodChecks(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void initGUI() {
		...
		pnlMain.setPreferredSize(new java.awt.Dimension(745, 487));
		{
			...
			panPage.setPreferredSize(new Dimension(0, 80));
			{
					...
					{
						{
							...
							// added by jari cruz asof may 10 2023 DCRF# 2581
							{
				...
				}
				{
					...
					{
						{
							JXPanel panPeriod = new JXPanel(new GridLayout(2, 2, 5, 5)); 
		...
		// modified by jari cruz dtd may 10 2023 DCRF# 2581
		fncGenerateTablePanel();
		fncGenerateSouthPanel();
	...
	public void displayListOfChecks(DefaultTableModel modelMain, JList rowHeader, DefaultTableModel modelTotal) {//
		...
		"and a.pay_part_id in (SELECT pay_part_id FROM mf_pay_particular where receipt_to_issue = '01' and status_id != 'I') \r\n" + 
		"and a.co_id = '"+co_id+"' " +
		...
		/*	Modified by Mann2x; Date Modified: July 6, 2017; DCRF# 156;		*/
	...
	private void executeRefresh() {
		 ...
		 */
		DisplayChecks display = new DisplayChecks();
	...
	private void save(){
//		if (rbtnA.isSelected()==true||rbtnB.isSelected()==true||rbtnC.isSelected()==true) {
...
//					+ "\n" + "Please ask assistance from SPT.", "Incomplete "+strType+"", JOptionPane.WARNING_MESSAGE);
//		} else {
//			if (checkCompleteDetails_assginedOR()==false) {
//				JOptionPane.showMessageDialog(getContentPane(), "Assigned "+strType+" not complete.", "Incomplete "+strType+"", JOptionPane.WARNING_MESSAGE);
//			} else {
	...
	private Boolean checkCompleteDetails(){
		...
		if (txtSuffixFrom.getText().equals("")) {
			if (JOptionPane.showConfirmDialog(pnlMain, "You did not assign the receipt suffix (A-Z). Proceed anyway?", 
		...
		if (!receipt_fr.equals("") && !receipt_to.equals("")) {
			blnDo = true; 
		}
	...
	private void issue1OR_perUnit(){
		...
		{		
			...
			try { pbl_desc_row 	= modelPaymentList.getValueAt(x,4).toString().trim(); } catch (NullPointerException e) { pbl_desc_row = ""; }

			if (pbl_desc_current.equals(pbl_desc_row))
			{
				last_receipt_no = String.format("%6s", receipt_assign).replace(' ', '0')+txtSuffixFrom.getText();
	...
	private void saveOR(pgSelect db){
		...
		if (rbtnD.isSelected()==true) {
			if (issueOR_perCheck(modelPaymentList)==true) {
			...
			if (issueOR_perCheck(modelPaymentList)==true) {
			//if (issueOR_perUnit(modelPaymentList)==true) { XXX PAKIBALIK ITO ONCE NA MATAPOS YUNG ISSUANCE NG 4 NA SI
				JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "New Official Receipts numbers are saved and issued.", "Save", JOptionPane.PLAIN_MESSAGE);
	...
	private Boolean issueOR_perCheck(modelIssueORGoodCheck model) {
		...
		String SQL = "SELECT sp_issueOR_goodCheck_perCheck(" +
	...
	private Object [] getPmtdetails(String pay_rec_id) {
		...
		String strSQL = 
				"select \r\n" + 
						...
						"a.client_seqno\r\n" + 		//7
						"\r\n" + 
						"from (select * from rf_payments where pay_rec_id = '"+pay_rec_id+"' and trim(status_id) = 'A') a" ;
	...
	private class DisplayChecks implements Runnable {

		public void run() {
			enableButtons(false, false, false, false);
			FncGlobal.startProgress("Generating list of checks...");
			displayListOfChecks(modelPaymentList, rowHeaderPaymentList, modelPaymentList_total);
	...
	private class previewReport implements Runnable {
	...
	private void preview_report() {
		...
		String strCompany = tagCompany.getText().replace("[", "");
		...
		System.out.println("cdr_date: " + dteDate.getDate().toString());
		System.out.println("cdr_date_str: " + dteDate.getDate().toString());
		...
		mapParameters.put("company", strCompany);
		mapParameters.put("co_id", lookupCompany.getValue());
		mapParameters.put("branch", "");
		mapParameters.put("logo", this.getClass().getClassLoader().getResourceAsStream("Images/"+ company_logo));
		mapParameters.put("prepared_by", UserInfo.FullName);
		mapParameters.put("cdr_date", dteDate.getDate());
		mapParameters.put("cdr_date_str", dteDate.getDate().toString());
		...
		FncReport.generateReport("/Reports/rptCRB_daily_preview_LateOR_v2.jasper", "Late OR CRB Detailed", strCompany, mapParameters);

		Map<String, Object> mapParameters1 = new HashMap<String, Object>();
		mapParameters1.put("company", strCompany);
		mapParameters1.put("co_id", lookupCompany.getValue());
		mapParameters1.put("branch", "");
		mapParameters1.put("logo", this.getClass().getClassLoader().getResourceAsStream("Images/"+ company_logo));
		mapParameters1.put("prepared_by", UserInfo.FullName);
		mapParameters1.put("cdr_date", dteDate.getDate());
		mapParameters1.put("cdr_date_str", dteDate.getDate().toString());
		mapParameters1.put("status", "POSTED");
		mapParameters1.put("posted_by", UserInfo.EmployeeCode );
		mapParameters1.put("proj_id", "");
		mapParameters1.put("phase", "");
		FncReport.generateReport("/Reports/rptCRB_daily_preview_summary_LateOR_v2.jasper", "Late OR CRB Summary", strCompany, mapParameters1);
	...
	private boolean ValidateSeries(String co_id) {
		...
		if(receipt_type.equals("307")) {
			return FncGlobal.GetBoolean("select (case when count(*) > 0 then true else false end) \n" + 
					"from \n" + 
					"( \n" + 
					"select x.or_no as receipt_no_char, replace(x.si_no, '"+txtSuffixFrom.getText()+"', '')::int as receipt_no_int from rf_payments x where (x.si_doc_id = '"+receipt_type+"' or x.si_date is not null) and (x.status_id = 'A' or (x.status_id = 'I' and request_no is not null)) and status_id = 'A' AND x.pay_rec_id NOT IN (62463) and x.co_id = '"+co_id+"'\n" + 
					"union \n" + 
					"select x.receipt_no as receipt_no_char, replace(x.receipt_no, '"+txtSuffixFrom.getText()+"', '')::int as receipt_no_int from rf_pay_detail x where x.receipt_type = '"+receipt_type+"' and x.status_id = 'A' and coalesce(x.receipt_no, '') != '' and status_id = 'A'\n" +
			...
			return FncGlobal.GetBoolean("select (case when count(*) > 0 then true else false end) \n" + 
					"from \n" + 
					"( \n" + 
					"select x.or_no as receipt_no_char, replace(x.or_no, '"+txtSuffixFrom.getText()+"', '')::int as receipt_no_int from rf_payments x where (x.or_doc_id = '"+receipt_type+"' or x.or_date is not null) and (x.status_id = 'A' or (x.status_id = 'I' and request_no is not null)) and status_id = 'A' AND x.pay_rec_id NOT IN (62463) and x.co_id = '"+co_id+"'\n" + 
					"union \n" + 
					"select x.receipt_no as receipt_no_char, replace(x.receipt_no, '"+txtSuffixFrom.getText()+"', '')::int as receipt_no_int from rf_pay_detail x where x.receipt_type = '"+receipt_type+"' and x.status_id = 'A' and coalesce(x.receipt_no, '') != '' and status_id = 'A'\n" +
					"and exists (SELECT * FROM rf_pay_header where client_seqno = x.client_seqno and co_id = '"+co_id+"') \n"+
					"union \n" + 
	...
	private void assignOR() {
		...
		if (rbtnA.isSelected()==false&&rbtnB.isSelected()==false&&rbtnC.isSelected()==false&&rbtnD.isSelected()==false) {
			JOptionPane.showMessageDialog(getContentPane(), "Please select method of "+strType+" issuance", "No Issuance Method", JOptionPane.WARNING_MESSAGE);
		} else {
			if (checkCompleteDetails()==false) {
				JOptionPane.showMessageDialog(getContentPane(), "Please enter complete receipt numbers", "Incomplete Detail", JOptionPane.WARNING_MESSAGE);
			} else {
	...
	private void issue1OR_perCheck_mod() {
		Integer receipt_fr = Integer.parseInt(txtReceiptFr_a.getText());
		Integer receipt_to = Integer.parseInt(txtReceiptTo_a.getText());
		Integer receipt_assign = 0;	
		Integer x = 0;
	...
	private Boolean issue1OR_perCheck() {
		...
		System.out.println("1 "+strType+" Per Check");

		try {
			for (int x=0; x < modelPaymentList.getRowCount(); x++) {
				Boolean isSelected = (Boolean) modelPaymentList.getValueAt(x, 15);
				Integer pay_rec_id = (Integer) modelPaymentList.getValueAt(x, 17);
				if (isSelected) {
					...
					if (intCurrent<intLast) {
						...
						System.out.printf("Last Receipt No.%s%n", last_receipt_no);
						modelPaymentList.setValueAt(last_receipt_no, x, 8);
					}	
				} else {
...

				{
...

					{
						...
						}	
						{
							...
							rbtnD.setText("1 "+strType+" per Check");
	...
	private void fncGenerateSouthPanel() {
		...
		
		{
			...
			pnlSouth.setPreferredSize(new java.awt.Dimension(1014, 30));
			{
				pnlSouthCenterb = new JPanel(new GridLayout(1, 4, 5, 5));
				pnlSouth.add(pnlSouthCenterb, BorderLayout.NORTH);
				pnlSouthCenterb.setPreferredSize(new java.awt.Dimension(921, 30));
				{
					{
