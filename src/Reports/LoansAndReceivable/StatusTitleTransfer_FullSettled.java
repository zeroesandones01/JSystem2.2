...
public class StatusTitleTransfer_FullSettled extends _JInternalFrame implements _GUI, ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7234015387514000667L;
	static String title = "Title Transfer Status - Full Settled";
	...
	public StatusTitleTransfer_FullSettled() {
		super(title, false, true, false, true);
		initGUI();
	}

	public StatusTitleTransfer_FullSettled(String title) {
		super(title, false, true, false, true);
		initGUI();
	}

	public StatusTitleTransfer_FullSettled(String title, boolean resizable, boolean closable,
		...
		this.setPreferredSize(frameSize);
		{
			...
			pnlMain.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
			{
				...
				}
				{
					...
					{
						JPanel pnldteTo = new JPanel(new BorderLayout(3, 3));
						pnlCenterComponents.add(pnldteTo);
						{
							dteTo = new _JDateChooser("MM/dd/yyyy", "##/##/####", '_');
							pnldteTo.add(dteTo, BorderLayout.WEST);
							//dteTo.setEditable(false);
							//dteTo.getCalendarButton().setEnabled(false);
							dteTo.setPreferredSize(new Dimension(150, 0));
							dteTo.setDate(Calendar.getInstance().getTime());
	...
	private String sqlCompanyLogo(String co_id){
		...
		String SQL = "SELECT company_logo \n"+
					 "FROM mf_company \n"+
					 "WHERE co_id = '"+co_id+"'";
		db.select(SQL);
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(actionCommand.equals("Preview")){
			...
			mapParameters.put("date_to", dteTo.getTimestamp());
			mapParameters.put("logo", this.getClass().getClassLoader().getResourceAsStream("Images/" + sqlCompanyLogo(lookupCompany.getValue())));
			
			FncReport.generateReport("/Reports/rptTitleTransferStatus_FullSettled.jasper","Title Transfer Status - Full Settled" , mapParameters);
		}
	}
}
