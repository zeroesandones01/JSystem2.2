...
public class panMBTCLoanReleased_Credit extends JXPanel implements _GUI {

	private static final long serialVersionUID = 1327332729872643154L;
	...
	public panMBTCLoanReleased_Credit(RealTimeDebitPiso_LoanReleased rtdPiso) {
		this.rtdMain = rtdPiso;
		initGUI();
	}

	public panMBTCLoanReleased_Credit(boolean isDoubleBuffered) {
		super(isDoubleBuffered);
		initGUI();
	}

	public panMBTCLoanReleased_Credit(LayoutManager layout) {
		super(layout);
		initGUI();
	}

	public panMBTCLoanReleased_Credit(LayoutManager layout, boolean isDoubleBuffered) {
								...
								public void actionPerformed(ActionEvent e) {
									...
									for (int x = 0; x < modelPiso.getRowCount(); x++) {
										if ((Boolean) modelPiso.getValueAt(x, 0)) {
											...
											strCountSQL = "select COUNT(*)::varchar from rf_rtd_piso_debit_loan_released where entity_id = '"+modelPiso.getValueAt(x, 9)+"' \n" +
											...
											strSQL = "insert into rf_rtd_piso_debit_loan_released (debit_rec_id, entity_id, proj_id, pbl_id, seq_no, accountno, trans_no, amount, date_created, created_by) \n" +
													"values ("+
													GetRecID() +", \n" +
	...
	public Boolean Generate() {
		...
		String strSQL = "select * from view_rtd_piso_debit_loan_released('"+strProID+"', '"+dteFrom.getDate()+"', '"+dteTo.getDate()+"') order by name";
    ...
    public static String glyphRTD() {
    	...
    	for(int x = 0;x < tblPiso.getRowCount();x++) {
    		if ((Boolean) tblPiso.getValueAt(x, 0)) {
    			...
    			Date dteDate = FncGlobal.GetDate("select date_opened::date from rf_loan_released_bank_details where entity_id = '"+(String) tblPiso.getValueAt(x, 9).toString()+"' and proj_id = '"+(String) tblPiso.getValueAt(x, 10).toString()+"' and pbl_id = '"+(String) tblPiso.getValueAt(x, 11).toString()+"' and seq_no::INT = '"+(String) tblPiso.getValueAt(x, 12).toString()+"'::INT");
    ...
    private void Preview() {
		...
		if (!blnWith) {
			...
			FncReport.generateReport("/Reports/rpt_MBTC_PisoDebit_LoanReleased.jasper", "Real-Time Debit List", "", mapParameters);
