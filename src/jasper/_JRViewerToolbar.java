 ...
 * @version $Id: _JRViewerToolbar.java,v 1.1 2019/04/10 00:30:01 jfatallo Exp $
 */
public class _JRViewerToolbar extends JPanel implements JRViewerListener {

	private static final Log log = LogFactory.getLog(_JRViewerToolbar.class);
	...
	protected String proj_id = null;
	protected String pbl_id = null;
	protected Integer seq_no = null;
	protected Boolean printable = false;

	public _JRViewerToolbar(JRViewerController viewerContext) {
	...
	public _JRViewerToolbar(JRViewerController viewerContext, String doc_id, String entity_id, String proj_id, String pbl_id, Integer seq_no, Boolean printable) {
		this.viewerContext = viewerContext;
		this.viewerContext.addListener(this);
		this.doc_id = doc_id;
	...
	private void initComponents() {//XXX initComponents
		...
		btnPrint.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {/*
				...
				if(!printable){
					//JOptionPane.showMessageDialog(_JRViewerToolbar.this.getTopLevelAncestor(), "This document has already printed.", "Print", JOptionPane.WARNING_MESSAGE);

					JPanel pnlHead = new JPanel(new GridLayout(6, 1, 5, 5));
					//pnlHead.setSize(200, 100);
					{
						JLabel lblHeader = new JLabel("<html>This document has already printed.<br>Department/Division Head can only re-print this document.</html>");
						pnlHead.add(lblHeader, BorderLayout.NORTH);
					}
					{//TODO prompt user to authorized by head
						pgSelect db = new pgSelect();
						db.select("SELECT a.emp_code, UPPER(FORMAT('%s %s', TRIM(b.first_name), TRIM(b.last_name))) as emp_name, a.emp_pos, b.*\n" + 
								"FROM em_employee a\n" + 
								"LEFT JOIN rf_entity b ON b.entity_id = a.entity_id\n" + 
								"WHERE a.dept_code IN (SELECT dept_code FROM em_employee WHERE emp_code = '"+ UserInfo.EmployeeCode +"')\n" + 
								"AND a.emp_status = 'R'\n" + 
								"AND a.emp_pos ~* '(OIC|HEAD)';");

						if(db.isNotNull()){
							for(int x=0; x < db.getRowCount(); x++){
								String emp_name = (String) db.getResult()[x][1];
								String emp_pos = (String) db.getResult()[x][2];

								pnlHead.add(new JLabel(String.format("   • %s (%s)", emp_name, emp_pos)), BorderLayout.NORTH);

								JPanel pnlUsername = new JPanel(new BorderLayout());
								pnlUsername.add(new JLabel("          Username: "), BorderLayout.WEST);
								pnlUsername.add(new JXTextField(), BorderLayout.CENTER);
								pnlUsername.add(new JLabel("          "), BorderLayout.EAST);
								pnlHead.add(pnlUsername);

								JPanel pnlPassword = new JPanel(new BorderLayout());
								pnlPassword.add(new JLabel("           Password: "), BorderLayout.WEST);
								pnlPassword.add(new JPasswordField(), BorderLayout.CENTER);
								pnlPassword.add(new JLabel("          "), BorderLayout.EAST);
								pnlHead.add(pnlPassword);
							}
						}
					}

					JOptionPane.showOptionDialog(_JRViewerToolbar.this.getTopLevelAncestor(), pnlHead, "Print", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, new Object[]{"OK", "Cancel"}, 1);
					return;
				}
		...
		txtSearchField.setToolTipText("Search");
		//txtSearchField.setMargin(new java.awt.Insets(2, 2, 2, 2));
		txtSearchField.setMaximumSize(new java.awt.Dimension(40, 23));
		txtSearchField.setMinimumSize(new java.awt.Dimension(40, 23));
		txtSearchField.setPreferredSize(new java.awt.Dimension(150, 23));
		txtSearchField.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				//txtGoToActionPerformed(evt);
				System.out.println("Search field here");
			}
		});
		add(txtSearchField);
		...
		if(!printable){
			JLabel lblWarning = new JLabel("  ***This document has already printed.***");
		...
		Thread thread = new Thread(new Runnable() {
			public void run() {
				try {
					...
					_JasperPrintManager.getInstance(viewerContext.getJasperReportsContext()).print(viewerContext.getJasperPrint(), true, doc_id, entity_id, proj_id, pbl_id, seq_no);
				} catch (Exception ex) {
					if (log.isErrorEnabled()) {
						log.error("Print error.", ex);
					}
					JOptionPane.showMessageDialog(_JRViewerToolbar.this, viewerContext.getBundleString("error.printing"));
				} finally {
	...
	void txtGoToActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtGoToActionPerformed
		try {
			...
			if (pageNumber != viewerContext.getPageIndex() + 1
					&& pageNumber > 0
	...
	protected void initSaveContributors() {
		List<JRSaveContributor> builtinContributors = SaveContributorUtils
				.createBuiltinContributors(
						viewerContext.getJasperReportsContext(), getLocale(),
	...
	protected void refreshPage() {//XXX Still Working
		...
		//btnSave.setEnabled(true); //printable
		btnPrint.setEnabled(true);
	...
	protected void zoomChanged() {
		cmbZoom.getEditor().setItem(
				zoomDecimalFormat.format(viewerContext.getZoom() * 100) + "%");
	}
