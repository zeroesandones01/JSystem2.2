...
public class rtdPisoDebit extends _JInternalFrame implements ActionListener {

	private static final long serialVersionUID = -7885415890465275589L;
	...
	public rtdPisoDebit(String title) {
		super(title);
		initGUI();
	}

	public rtdPisoDebit(String title, boolean resizable, boolean closable,
	...
	private void CreatePage() {
		...
		panGrid.setOpaque(isOpaque());
		{
			...
			scrPiso.setBorder(lineGray);
			{
				...
				tblPiso.getColumnModel().getColumn(2).setPreferredWidth(70);
				...
				tblPiso.hideColumns("entity_id", "proj_id", "pbl_id", "seq_no");
			}
			{
				rowPiso = tblPiso.getRowHeader();
	...
	public void actionPerformed(ActionEvent act) {
		if (act.getActionCommand().equals("Process")) {
			if (Generate()) {
				buttonState(true, true, true);
			} else {
				JOptionPane.showMessageDialog(getContentPane(), "No records were retrieved.");
		...
		} else if (act.getActionCommand().equals("Dir")) {
    ...
    public static String glyphRTD() {
		...
		String strDate = dteFormat.format(date);
