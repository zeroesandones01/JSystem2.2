...
public class SearchFactory {
	 ...
	 */
	public void showFindInput(JComponent target, /*Searchable searchable*/JXTable table, int column) {
		if (isUseFindBar(target, table.getSearchable())) {
			showFindBar(target, table.getSearchable());
		} else {
			showFindDialog(target, table, column);
		}
	}
	 ...
	 * @param searchable - the object to search.
	 */
	public void showFindBar(JComponent target, Searchable searchable) {//XXX showFindBar
		...
		getSharedFindBar().setSearchable((org.jdesktop.swingx.search.Searchable) searchable);
	 ...
	 */
	public void showFindDialog(JComponent target, /*Searchable searchable, */JXTable table, int column) {//XXX showFindDialog
		...
		_JXDialog topLevel = getDialogForSharedFindPanel();
		_JXDialog findDialog;
		if ((topLevel != null) && (topLevel.getOwner().equals(frame))) {
			...
			if (frame instanceof Frame) {
				...
				findDialog = new _JXDialog((Frame) frame, getSharedFindPanel(table, column));
			} else if (frame instanceof Dialog) {
				System.out.println("**********#2");
				// fix #215-swingx: had problems with secondary modal dialogs.
				findDialog = new _JXDialog((Dialog) frame, getSharedFindPanel(table, column));
			} else {
				System.out.println("**********#3");
				findDialog = new _JXDialog(JOptionPane.getRootFrame(), getSharedFindPanel(table, column));
			}
		...
		getSharedFindPanel(table, column).setSearchable(table.getSearchable());
	 ...
	 */
	public JXFindPanel getSharedFindPanel(JXTable table, int column) {
		if (findPanel == null) {
			findPanel = createFindPanel(table, column);
	 ...
	 */
	public JXFindPanel createFindPanel(JXTable table, int column) {
		return new JXFindPanel(table, column);
	}
	...
	private _JXDialog getDialogForSharedFindPanel() {
		if (findPanel == null) return null;
		Window window = SwingUtilities.getWindowAncestor(findPanel);
		return (window instanceof _JXDialog) ? (_JXDialog) window : null;
	}
