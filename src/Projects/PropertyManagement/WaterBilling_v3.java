...
public class WaterBilling_v3 extends _JInternalFrame implements _GUI, ActionListener, MouseListener {
	...
	public WaterBilling_v3() {
		super(title, true, true, true, true);
		initGUI();
	}

	public WaterBilling_v3(String title) {
		super(title);
		// TODO Auto-generated constructor stub
	}

	public WaterBilling_v3(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void initGUI() {
...

		{
			...
			mniViewBill.addActionListener(new ActionListener(){
				public void	actionPerformed(ActionEvent evt){
					...
					pbl_id 	 = "";
					entity_id = "";		
					try { pbl_id = modelWBilling.getValueAt(row, 8).toString().replace(",",""); } catch (NullPointerException e) { pbl_id = "" ; }
					try { entity_id = modelWBilling.getValueAt(row, 9).toString().replace(",",""); } catch (NullPointerException e) { entity_id = "" ; }		
...

		{
			...
			pnlNorth.setPreferredSize(new java.awt.Dimension(923, 195));				
			...
			pnlRepair.setPreferredSize(new java.awt.Dimension(921, 76));
			pnlRepair.setBorder(JTBorderFactory.createTitleBorder("Meter Reading Details"));

			{

				pnlMeterReaderDtl = new JPanel(new BorderLayout(5, 5));
				pnlRepair.add(pnlMeterReaderDtl, BorderLayout.WEST);	
				pnlMeterReaderDtl.setPreferredSize(new java.awt.Dimension(841, 99));

				pnlMeterReaderDtl_1 = new JPanel(new BorderLayout(5, 5));
				pnlMeterReaderDtl.add(pnlMeterReaderDtl_1, BorderLayout.WEST);	
				pnlMeterReaderDtl_1.setPreferredSize(new java.awt.Dimension(211, 94));
				pnlMeterReaderDtl_1.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 0));

				pnlMeterReaderDtl_1a = new JPanel(new GridLayout(2, 1, 5, 5));
				pnlMeterReaderDtl_1.add(pnlMeterReaderDtl_1a, BorderLayout.WEST);	
				pnlMeterReaderDtl_1a.setPreferredSize(new java.awt.Dimension(89, 119));
				pnlMeterReaderDtl_1a.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));	

				{
					lblReader = new JLabel("Meter Reader", JLabel.TRAILING);
					pnlMeterReaderDtl_1a.add(lblReader);
					lblReader.setEnabled(true);	
				}	
				{
					lblDate = new JLabel("Date", JLabel.TRAILING);
					pnlMeterReaderDtl_1a.add(lblDate);
					lblDate.setEnabled(true);	
				}

				pnlMeterReaderDtl_1b = new JPanel(new GridLayout(2, 1, 5, 5));
				pnlMeterReaderDtl_1.add(pnlMeterReaderDtl_1b, BorderLayout.CENTER);	
				pnlMeterReaderDtl_1b.setPreferredSize(new java.awt.Dimension(107, 84));
				pnlMeterReaderDtl_1b.setBorder(BorderFactory.createEmptyBorder(0, 5, 0, 5));

				{
					lookupReader = new _JLookup(null, "Reader",0,2);
					pnlMeterReaderDtl_1b.add(lookupReader);
				...
				}	
				{
					...
					pnlMeterReaderDtl_1b.add(dateReading, BorderLayout.EAST);
					dateReading.setBounds(485, 7, 125, 21);
					dateReading.setDate(null);
					dateReading.setEnabled(true);
					dateReading.setDate(FncGlobal.dateFormat(FncGlobal.getDateSQL()));
				}		

				pnlMeterReaderDtl_2 = new JPanel(new GridLayout(2, 1, 5, 5));
				pnlMeterReaderDtl.add(pnlMeterReaderDtl_2, BorderLayout.CENTER);	
				pnlMeterReaderDtl_2.setPreferredSize(new java.awt.Dimension(676, 99));
				pnlMeterReaderDtl_2.setBorder(BorderFactory.createEmptyBorder(5, 0, 5, 0));

				{
					tagReader = new _JTagLabel("[ ]");
					pnlMeterReaderDtl_2.add(tagReader);
					tagReader.setEnabled(true);	
				}
				{
					pnlMeterReaderDtl_2_a = new JPanel(new BorderLayout(0,0));
					pnlMeterReaderDtl_2.add(pnlMeterReaderDtl_2_a, BorderLayout.CENTER);	
					pnlMeterReaderDtl_2_a.setPreferredSize(new java.awt.Dimension(309, 123));						

					pnlMeterReaderDtl_2_a1 = new JPanel(new GridLayout(1, 1, 5, 5));
					pnlMeterReaderDtl_2_a.add(pnlMeterReaderDtl_2_a1, BorderLayout.WEST);	
					pnlMeterReaderDtl_2_a1.setPreferredSize(new java.awt.Dimension(110, 42));

					{	
						Color maroon = new Color (128, 0, 0);

						lblClientName = new JLabel("Search Client : ", JLabel.TRAILING);
						pnlMeterReaderDtl_2_a1.add(lblClientName);
						lblClientName.setEnabled(true);	
						lblClientName.setPreferredSize(new java.awt.Dimension(98, 42));
						lblClientName.setFont(new java.awt.Font("Segoe UI",Font.ITALIC + Font.BOLD,12));
						lblClientName.setForeground(maroon);
					}	

					pnlMeterReaderDtl_2_a2 = new JPanel(new GridLayout(1, 1, 5, 5));
					pnlMeterReaderDtl_2_a.add(pnlMeterReaderDtl_2_a2, BorderLayout.CENTER);	
					pnlMeterReaderDtl_2_a2.setPreferredSize(new java.awt.Dimension(309, 123));

					{
						txtClientName= new JXTextField("");
						pnlMeterReaderDtl_2_a2.add(txtClientName);
			...
			{			
...

				{
					...
					pnTableRepair.add(scrollMeterReader, BorderLayout.CENTER);
					{
						...
						tblWBilling.addMouseListener(new MouseAdapter() {
							public void mousePressed(MouseEvent e) {
								if(tblWBilling.rowAtPoint(e.getPoint()) == -1){
									tblWBilling_total.clearSelection();
								}else{
									tblWBilling.setCellSelectionEnabled(true);
								}
								displayAddReading();
							}
	...
	public void displayWBilling(DefaultTableModel modelMain, JList rowHeader, DefaultTableModel modelTotal) {   //used
		...
		String sql = 
				...
				"select c_unit, c_client_name, c_prev_reading, c_curr_reading, c_water_cost, c_garbage_fee, c_amt_due, c_remarks, c_pbl_id, c_entity_id, c_proj_id, c_seq_no from display_water_billing_units('"+co_id+"', '"+proj_id+"','"+ph_no+"','"+dateReading.getDate().toString()+"')";
		...
		if(db.isNotNull()){
			...
			System.out.printf("Display Water Billing Accounts SQL", sql);

			totalRepair(modelMain, modelTotal);	

		}		

		else {modelWBilling_total.addRow(new Object[] { "Total", null, null, null, new BigDecimal(0.00), 
				new BigDecimal(0.00), new BigDecimal(0.00), null });}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("Generate")){ generate(); }

		if(e.getActionCommand().equals("Refresh")){ refresh(); }

		if(e.getActionCommand().equals("Save")){ save(); }

		if(e.getActionCommand().equals("Preview")){ pbl_id 	 = "";  entity_id = ""; preview();}
	}
	...
	private void totalRepair(DefaultTableModel modelMain, DefaultTableModel modelTotal) {
		...
		for(int x=0; x < modelMain.getRowCount(); x++){		
			...
			catch (NullPointerException e) { reading =0.00 ; }			
		...
		if (lookupReader.getText().equals("")){
			...
			if (JOptionPane.showConfirmDialog(getContentPane(), "Are all entries correct?", "Save", 
				...
				for(int x =0; x < modelWBilling.getRowCount(); x++){
					...
					try { reading = modelWBilling.getValueAt(x, 3).toString().replace(",","");} 	
					catch (NullPointerException e1) { reading = "" ; }	

					if (!reading.equals(""))
					{
						String pbl = (String) modelWBilling.getValueAt(x, 8);
						Double prev_reading = Double.parseDouble(modelWBilling.getValueAt(x, 2).toString().replace(",",""));
						Double curr_reading = Double.parseDouble(modelWBilling.getValueAt(x, 3).toString().replace(",",""));
						Double water_cost 	= Double.parseDouble(modelWBilling.getValueAt(x, 4).toString().replace(",",""));

						String entity = (String) modelWBilling.getValueAt(x, 9);
						String reader = (String) lookupReader.getText();
						Date reading_date = (Date) dateReading.getDate();
						String remarks = (String) modelWBilling.getValueAt(x, 7);
						String seq_no = modelWBilling.getValueAt(x, 11).toString();
