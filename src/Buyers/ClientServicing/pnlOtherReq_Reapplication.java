 ...
 *
 */
public class pnlOtherReq_Reapplication extends JPanel implements _GUI {
	...
	public pnlOtherReq_Reapplication(pnlOtherRequest_OldData od) {
		this.od = od;
		initGUI();
	}

	public pnlOtherReq_Reapplication(LayoutManager layout) {
		super(layout);
	}

	public pnlOtherReq_Reapplication(boolean isDoubleBuffered) {
	...
	public void initGUI() {
		...
		this.setPreferredSize(new Dimension(200, 200));
		{
			...
			{
				pnlWest = new JPanel(new GridLayout(9, 1, 3, 3));
			...
			{
				pnlEast = new JPanel(new GridLayout(9, 1, 3, 3));
				pnlMain.add(pnlEast, BorderLayout.CENTER);
				{
					...
					pnlEast.add(pnlSellingAgent);
					{
						...
						lookupAgent.setLookupSQL(_OtherRequest2.sqlChangeAgent("07" ,od.getDataOld()));
						...
						lookupAgent.addResetListener(new ResetListener() {
							
							@Override
							public void resetPerformed(ResetEvent event) {
								lookupAgent.setValue(null);
								txtAgent.setText("");
							}
						});
					}
					...
					pnlEast.add(pnlPmtScheme);
					{
						...
						lookupPmtScheme.addLookupListener(new LookupListener() {
							...
							public void lookupPerformed(LookupEvent event) {
								Object [] data = ((_JLookup)event.getSource()).getDataSet();
								if(data!= null){
									Object [] data3 = _OtherRequest2.setFinalVariables(od.getDataOld(), getDataReapplication());
						...
						txtMATerm.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent arg0) {
								try{
									...
									if(ma_term % 12 != 0){
										JOptionPane.showMessageDialog(pnlOtherReq_Reapplication.this.getTopLevelAncestor(), "Please input a number divisible by twelve");
										txtMATerm.setText("");
									}
	...
	public void computeAmounts(Object [] data3){ //Copy To Other Modules
		...
		if(_OtherRequest2.getGroupID(final_entity_class).equals("04")){//FOR PAGIBIG ACCOUNTS
			String sql = "select round(loanable_amt, 2), round(((loanable_amt/"+old_sprice+")*100), 2), \n"+
					""+old_sprice+"-loanable_amt, 100-round(((loanable_amt/"+old_sprice+")*100), 2)"
			...
			txtDiscAmt.setValue(new BigDecimal("0.00"));
			txtDiscRate.setValue(new BigDecimal("0.00"));
			txtNetSellingPrice.setValue(old_sprice);
	...
	public void displayReapplication(String new_entity_class, String new_entity_class_desc, String new_pmt_scheme, String new_pmt_scheme_desc,
			BigDecimal new_disc_amt, BigDecimal new_disc_rate, BigDecimal new_dp_amt, BigDecimal new_dp_rate, Integer new_dpterm,
			BigDecimal new_availed_amt, BigDecimal new_availed_rate, Integer new_materm, BigDecimal new_sprice, String new_agent_id, String new_agent_name){
	...
	public Object[] getDataReapplication(){
		return new Object[]{"07", //req_id 0
