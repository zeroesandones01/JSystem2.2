 ...
 * 
 */
public class TransferJournalEntries extends _JInternalFrame implements
		ActionListener {
	...
	private void initGUI() {
		...
		this.setLayout(new BorderLayout());
		{
			...
			pnlMain.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
			{
				...
				}
				{
					...
					lookupPhase.addLookupListener(new LookupListener() {
						public void lookupPerformed(LookupEvent event) {
							...
							if (data != null) {
								...
								DESTINATION_DATABASE = "_verdant_"
										+ tagPhaseAlias.getText().toLowerCase();
								DESTINATION_URL = "jdbc:postgresql://"
										+ DESTINATION_HOST + ":"
										+ DESTINATION_PORT + "/"
										+ DESTINATION_DATABASE + "?user="
										+ DESTINATION_USERNAME + "&password="
					...
					lookupPhase.addFocusListener(new FocusListener() {
						...
						public void focusGained(FocusEvent e) {
							_JLookup lookup = (_JLookup) e.getSource();
							lookup.setReturnColumn(0);
							lookup.setLookupSQL(FncGlobal
									.getPhase(lookupProject.getValue()));
						}
					});
				...
				{
					cmbBooks = new JComboBox(new DefaultComboBoxModel(
							new String[] { "All Books", "Receipt Book",
									"Cash Voucher", "Journal Voucher" }));
				...
				}
				{
					...
					dateFrom.setDate(FncDate.getFirstDateOfMonth(Calendar
							.getInstance().getTime()));
					dateFrom.setBounds(98, 152, 104, 25);
					dateFrom.addDateListener(new DateListener() {
						public void datePerformed(DateEvent e) {
							_JDateChooser dateChooser = (_JDateChooser) e
									.getSource();
							FncDate.setLastDayOfMonth(dateChooser.getDate(),
									dateTo);
						}
					});
				}
			...
			}
			{
				...
				}
				{
					...
					scrollMainSouth.setBorder(BorderFactory.createEmptyBorder(
							0, 1, 0, 17));
					scrollMainSouth.getHorizontalScrollBar()
							.addAdjustmentListener(new AdjustmentListener() {
								public void adjustmentValueChanged(
										AdjustmentEvent e) {
									scrollMainCenter.getHorizontalScrollBar()
											.setValue(e.getValue());
								}
							});
					scrollMainSouth.setRowHeaderView(FncTables
							.getRowHeader_Footer("0"));
					scrollMainSouth.setPreferredSize(new Dimension(586, 37));
					{
						_modelTotal = new tablemodelTransferJournalEntries();
						_modelTotal.addRow(new Object[] { "Total", null, 0.00,
								0.00 });
						...
						_tblTotal.setDefaultRenderer(Object.class, TextRenderer
								.getTextRenderer(SwingConstants.CENTER));
						_tblTotal.setDefaultRenderer(BigDecimal.class,
								NumberRenderer.getMoneyRenderer());
						_tblTotal.setFillsViewportHeight(false);

						_tblTotal.addHighlighter(FncTables
								.getHighlighterTOTAL(0));
						...
						_tblMain.getColumnModel().addColumnModelListener(
								new TableColumnModelListener() {
									...
									public void columnMarginChanged(
											ChangeEvent arg0) {
										TableColumnModel tableColumnModel = _tblMain
												.getColumnModel();
										TableColumnModel footerColumnModel = _tblTotal
												.getColumnModel();
										for (int i = 0; i < tableColumnModel
												.getColumnCount(); i++) {
											int w = tableColumnModel.getColumn(
													i).getWidth();
											footerColumnModel.getColumn(i)
													.setMinWidth(w);
											footerColumnModel.getColumn(i)
													.setMaxWidth(w);
											footerColumnModel.getColumn(i)
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("Display")) {
			...
			case 0:
				_TransferJournalEntries.displayAll(_modelMain, _modelTotal, rowHeader, projectName, lookupCompany.getValue(), dateFrom.getDate(), dateTo.getDate(), project);
				break;
			case 1:
				_TransferJournalEntries.displayRB(_modelMain, _modelTotal, rowHeader, projectName, lookupCompany.getValue(), dateFrom.getDate(), dateTo.getDate(), project);
				break;
			case 2:
				_TransferJournalEntries.displayCV(_modelMain, _modelTotal, rowHeader, projectName, lookupCompany.getValue(), dateFrom.getDate(), dateTo.getDate(), project);
				break;
			case 3:
				_TransferJournalEntries.displayJV(_modelMain, _modelTotal, rowHeader, projectName, lookupCompany.getValue(), dateFrom.getDate(), dateTo.getDate(), project);
				break;
			default:
				break;
			}
