...
import tablemodel.modelTB_generate;
...
public class TrialBalance extends _JInternalFrame implements _GUI, ActionListener, MouseListener {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3061284418918863916L;
	static String title = "Trial Balance (TB)";
	...
	private static JMenuItem mniCloseTB;
	...
	private static JComboBox cmbPeriodFr;
	private static JComboBox cmbPeriodTo;
	private static JComboBox cmbInclude;
	private static JComboBox cmbYear;

	private static modelTB_generate modelTB_details;
	private static modelTB_generate modelTB_details_total;	
	...
	public TrialBalance(String title) {
		super(title);
		// TODO Auto-generated constructor stub
	}

	public TrialBalance(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void initGUI() {
		...
		pnlNorth_main.setPreferredSize(new java.awt.Dimension(1000, 160));
...

		{
			...
			{			
				...
				tabCenter.addTab("   TB Details   ", null, pnlTBtable_acct_entries, null);
...

				{
					...
					{
						modelTB_details = new tablemodel.modelTB_generate();
				...
				}
				{
					...
					{
						modelTB_details_total = new modelTB_generate();
	...
	//display tables
	public static void generateTB(DefaultTableModel modelMain, JList rowHeader, DefaultTableModel modelTotal, String statusID) {
		...
		if(UserInfo.EmployeeCode.equals("900876")) {
			...
			sql = "select * from view_trial_balance_detailed_debug('"+co_id+"','"+ph_no+"','"+ph_code+"','"+date_from+"'," +
					"'"+date_to+"', '"+proj_id+"', "+show_zero+",'"+statusID+"','"+include_month+"',"+period_from+","+period_to+" ) ";
		}else {

			/*sql = 	"select * from view_trial_balance_detailed_debug('"+co_id+"','"+ph_no+"','"+ph_code+"','"+date_from+"'," +
					"'"+date_to+"', '"+proj_id+"', "+show_zero+",'"+statusID+"','"+include_month+"',"+period_from+","+period_to+" ) ";*/
			sql = 	"select * from view_trial_balance_detailed_debug_erick('"+co_id+"','"+ph_no+"','"+ph_code+"','"+date_from+"'," +
					"'"+date_to+"', '"+proj_id+"', "+show_zero+",'"+statusID+"','"+include_month+"',"+period_from+","+period_to+" ) ";
		}
		...
		if(db.isNotNull()){
			...
			modelTB_details_total = new tablemodel.modelTB_generate();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("Preview")){
			...
			//}
				// Added by Erick 2023-10-06 DCRF 2774
				Object[] option= {"Summary Report","Detailed Report"};
				int Option=JOptionPane.showOptionDialog(getContentPane(), "Please select report.", "Report Option", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, option, option[0]);
	...
	public static void totalTB(DefaultTableModel modelMain, DefaultTableModel modelTotal) {
		...
		BigDecimal beg_bal 	= new BigDecimal(0.00);	
		BigDecimal crb_bd 	= new BigDecimal(0.00);	
		BigDecimal cv_bd 	= new BigDecimal(0.00);	
		BigDecimal pv_bd 	= new BigDecimal(0.00);	
		BigDecimal jv_bd 	= new BigDecimal(0.00);	
		BigDecimal per_tran 	= new BigDecimal(0.00);
		BigDecimal end_bal 	= new BigDecimal(0.00);

		for(int x=0; x < modelMain.getRowCount(); x++){		

			try { beg_bal 	= beg_bal.add(((BigDecimal) modelMain.getValueAt(x,2)));} 
			catch (NullPointerException e) { beg_bal 	= beg_bal.add(new BigDecimal(0.00)); }

			try { crb_bd 	= crb_bd.add(((BigDecimal) modelMain.getValueAt(x,3)));} 
			catch (NullPointerException e) { crb_bd 	= crb_bd.add(new BigDecimal(0.00)); }

			try { cv_bd = cv_bd.add(((BigDecimal) modelMain.getValueAt(x,4)));} 
			catch (NullPointerException e) { cv_bd 	= cv_bd.add(new BigDecimal(0.00)); }

			try { pv_bd = pv_bd.add(((BigDecimal) modelMain.getValueAt(x,5)));} 
			catch (NullPointerException e) { pv_bd 	= pv_bd.add(new BigDecimal(0.00)); }

			try { jv_bd = jv_bd.add(((BigDecimal) modelMain.getValueAt(x,6)));} 
			catch (NullPointerException e) { jv_bd 	= jv_bd.add(new BigDecimal(0.00)); }

			try { per_tran = per_tran.add(((BigDecimal) modelMain.getValueAt(x,7)));} 
			catch (NullPointerException e) { per_tran 	= per_tran.add(new BigDecimal(0.00)); }

			try { end_bal = end_bal.add(((BigDecimal) modelMain.getValueAt(x,8)));} 
			catch (NullPointerException e) { end_bal 	= end_bal.add(new BigDecimal(0.00)); }

		}		

		modelTotal.addRow(new Object[] { "Total", null, beg_bal, crb_bd, cv_bd, pv_bd, jv_bd, per_tran, end_bal });
	}
	...
	public static void openGL(){
		...
		{		
			...
			GeneralLedger.period_from 	= period_from;
			GeneralLedger.period_to 	= period_to;	
			GeneralLedger.date_from 	= date_from;
			GeneralLedger.date_to 		= date_to;
			...
			if (rbtnDate.isSelected()==true){
				GeneralLedger.rbtnDate.setSelected(true);
				GeneralLedger.lbl_date_fr.setEnabled(true);	
				GeneralLedger.dte_from.setEnabled(true);
				GeneralLedger.lblDate_to.setEnabled(true);
				GeneralLedger.dte_to.setEnabled(true);
				GeneralLedger.lblInclude.setEnabled(true);
				GeneralLedger.cmbInclude.setEnabled(true);

				GeneralLedger.rbtnPeriod.setSelected(false);
				GeneralLedger.lbl_period_fr2.setEnabled(false);	
				GeneralLedger.cmbPeriodFr.setEnabled(false);
				GeneralLedger.lblPeriod_to2.setEnabled(false);
				GeneralLedger.cmbPeriodTo.setEnabled(false);
				GeneralLedger.lblYear.setEnabled(false);
				GeneralLedger.cmbYear.setEnabled(false);
			}
			else 
			{		
				GeneralLedger.rbtnDate.setSelected(false);
				GeneralLedger.lbl_date_fr.setEnabled(false);	
				GeneralLedger.dte_from.setEnabled(false);
				GeneralLedger.lblDate_to.setEnabled(false);
				GeneralLedger.dte_to.setEnabled(false);
				GeneralLedger.lblInclude.setEnabled(false);
				GeneralLedger.cmbInclude.setEnabled(false);

				GeneralLedger.rbtnPeriod.setSelected(true);
				GeneralLedger.lbl_period_fr2.setEnabled(true);	
				GeneralLedger.cmbPeriodFr.setEnabled(true);
				GeneralLedger.lblPeriod_to2.setEnabled(true);
				GeneralLedger.cmbPeriodTo.setEnabled(true);
				GeneralLedger.lblYear.setEnabled(true);
				GeneralLedger.cmbYear.setEnabled(true);
			}					
			...
			GeneralLedger.cmbYear.setSelectedIndex(cmbYear.getSelectedIndex());
			GeneralLedger.cmbPeriodFr.setSelectedIndex(cmbPeriodFr.getSelectedIndex());
			GeneralLedger.cmbPeriodTo.setSelectedIndex(cmbPeriodTo.getSelectedIndex());

			DefaultListModel listModel = new DefaultListModel();
			GeneralLedger.getDate();
	...
	public static void openJV_closeTB(String dte_fr, String dte_to){
		...
		if(checkClosable_v2(dte_fr, dte_to)==false)
	...
	public static Boolean checkClosable(String dte_fr, String dte_to){		
		...
		String SQL = 
				...
				"		and co_id = '"+co_id+"' \n" +
				"		and (rb_id, doc_id) not in (		 \n" + 
				"			select or_no, (case when or_date is null then '03' else '01' end) \n" + 
				"			from rf_payments \n" + 
				...
				"			select or_no, (case when or_date is null then '03' else '01' end)\n"+
				"			from rf_payments \n"+
				"			where remarks like '%Special Case%' and trans_date::date >= '2018-01-01'::date \n"+
				"			union all\n"+
				"			select ar_no, '03'\n" + 
				"			from rf_payments \n" + 
				"			where request_no is not null \n" + 
				"			and to_char(trans_date,'yyyy')::int >= 2018 \n" + 
				"			and ar_no is not null\n" + 
				"			\n" + 
				"			) \n" +
				"		and rb_id not in ('013056','017835','025343','013111') \n" +  //this is a special condition to filter out abnormal ORs; 
				"	union \n" + 
	...
	//Added by Erick 2023-09-20 new version of checkClosable
	public static Boolean checkClosable_v2(String dte_fr, String dte_to){		
		...
		String SQL = 
				...
				+ "	and issued_date::date <= '"+dte_to+"'::date \n"
				+ "	and co_id = '"+co_id+"'\n"
				+ "	AND NOT EXISTS (select * from issued_garbage_fee where client_seqno = reference_no)\n"
				...
				+ "		where\n"
				+ "		(\n"
				+ "			case when to_char('"+dte_fr+"'::date,'yyyy')::int <= 2017 -- gl diff\n"
				...
				+ "	) \n"
				+ "and rb_id not in ('013056','017835','025343','013111', \n"
				+ " '020313B') \n" //Added by erick 2023-009-20 as requested by orly
				+ "and rb_id not in ('006037B','006035B','006019B','006036B','005988','005987','006018B')\n"// Added by erick 2023-09-25 DCRF 2157
				+ "union \n"
				+ "select jv_no from rf_jv_header \n"
				+ "	where status_id in ('A', 'F') \n"
				+ "	and jv_date::date >= '"+dte_fr+"'::date \n"
