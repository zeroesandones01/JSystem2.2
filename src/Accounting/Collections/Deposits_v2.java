...
import tablemodel.modelCashClientList;
...
public class Deposits_v2 extends _JInternalFrame implements _GUI, ActionListener, MouseListener {
	...
	private _JTableMain tblCashClientList;
	...
	private modelCashClientList modelCashList;
	...
	public Deposits_v2(String title) {
		super(title);

	}

	public Deposits_v2(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void initGUI() {
						...
						lookupDepositNo.addLookupListener(new LookupListener() {
							public void lookupPerformed(LookupEvent event) {
								...
								if (data != null) {
									...
									displayCashClients(modelCashList, rowHeaderClientList, lookupCompany.getValue(), lookupOfficeBranch.getValue(), dteCollection.getDate(), dep_no);
							...
							SwingWorker sw = new SwingWorker() {
								protected Object doInBackground()
									...
									txtJVRemark.setText("TO RECORD DEPOSIT FOR "
											...
											+ dateFormat.format(dteCollection.getDate()));
									FncGlobal.stopProgress();
									displayCashClients(modelCashList, rowHeaderClientList, lookupCompany.getValue(), lookupOfficeBranch.getValue(), dteCollection.getDate(), null);
									return null;
								}
							};
							sw.execute();
...

		{
			...
			// Cash Deposit
			{
				...
				{
					JPanel pnlCashDepositWest = new JPanel(new BorderLayout(5, 5));
					pnlSample1.add(pnlCashDepositWest, BorderLayout.WEST);
					pnlCashDepositWest.setPreferredSize(new Dimension(400, 0));
					pnlCashDepositWest.setBorder(JTBorderFactory.createTitleBorder("Denominations"));
					{
						scrollSample1Main = new _JScrollPaneMain();
						pnlCashDepositWest.add(scrollSample1Main);
						//scrollSample1Main.setPreferredSize(new Dimension(400, 0));
						{
					...
					}
					{
						...
						pnlCashDepositWest.add(scrollSample1Total, BorderLayout.SOUTH);
						{
				...
				{
					JPanel pnlCashDepositCenter = new JPanel(new BorderLayout(5, 5));
					pnlSample1.add(pnlCashDepositCenter, BorderLayout.CENTER);
					pnlCashDepositCenter.setBorder(JTBorderFactory.createTitleBorder("Client List"));
					{
						scrollClientList = new _JScrollPaneMain();
						pnlCashDepositCenter.add(scrollClientList);
						//scrollSample1Main.setPreferredSize(new Dimension(400, 0));
						{
							modelCashList = new modelCashClientList();

							tblCashClientList = new _JTableMain(modelCashList);
							scrollClientList.setViewportView(tblCashClientList);
							tblCashClientList.addMouseListener(this);
							tblCashClientList.setSortable(false);
							tblCashClientList.hideColumns("Pay Rec ID");

							tblCashClientList.addMouseListener(new MouseAdapter() {
								public void mousePressed(MouseEvent e) {
									if (tblCashClientList.rowAtPoint(e.getPoint()) == -1) {
										tblSample1Total.clearSelection();
									} else {
										tblCashClientList.setCellSelectionEnabled(true);
									}
								}
							...
							tblCashClientList.addPropertyChangeListener("tableCellEditor", new PropertyChangeListener() {
								public void propertyChange(PropertyChangeEvent arg0) {
									_JTableMain table = (_JTableMain) arg0.getSource();
						...
						{
							rowHeaderClientList = tblCashClientList.getRowHeader();
							scrollClientList.setRowHeaderView(rowHeaderClientList);
							scrollClientList.setCorner(ScrollPaneConstants.UPPER_LEFT_CORNER,
									FncTables.getRowHeader_Header());
						}
					}
				}
	...
	private void displayCashClients(DefaultTableModel modelMain, JList rowHeader, String co_id, String branch_id, Date collection_date, String dep_no) {
		FncTables.clearTable(modelMain);
		DefaultListModel listModel = new DefaultListModel();
		rowHeader.setModel(listModel);
		
		pgSelect db = new pgSelect();
		String SQL = "SELECT * FROM view_cash_deposit_list('"+co_id+"', '"+branch_id+"', '"+collection_date+"', NULLIF('"+dep_no+"', 'null'))";
		...
		modelCashList.setEditable(true);
		tblCashClientList.packAll();
	}
	...
	private void addnew() {
		...
		//displayCashClients(modelCashList, rowHeaderClientList, lookupCompany.getValue(), lookupOfficeBranch.getValue(), dteCollection.getDate());
		pnlBank.setBorder(JTBorderFactory.createTitleBorder("Bank Info."));
...
//				dteDeposit.getDate());
	...
	private void saveCashPmtDetails(String dep_no) {
		
		pgSelect db = new pgSelect();
		ArrayList<Integer> listPayRecID = new ArrayList<Integer>();
		
		for(int x= 0; x<modelCashList.getRowCount(); x++) {
			Boolean isSelected = (Boolean) modelCashList.getValueAt(x, 0);
			
			if(isSelected) {
				Integer pay_rec_id = (Integer) modelCashList.getValueAt(x, 6);
		...
		String SQL = "SELECT sp_tag_cash_dep_pmts('"+dep_no+"', ARRAY["+pay_rec_id+"]::INT[], '"+UserInfo.EmployeeCode+"')";
		db.select(SQL);
