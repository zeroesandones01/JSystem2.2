...
public class WaterBilling_v2 extends _JInternalFrame implements _GUI, ActionListener, MouseListener {
	...
	public WaterBilling_v2() {
		super(title, true, true, true, true);
		initGUI();
	}

	public WaterBilling_v2(String title) {
		super(title);
		// TODO Auto-generated constructor stub
	}

	public WaterBilling_v2(String title, boolean resizable, boolean closable, boolean maximizable, boolean iconifiable) {
	...
	public void displayWBilling(DefaultTableModel modelMain, JList rowHeader, DefaultTableModel modelTotal) {   //used
		...
		String sql = 
				...
				"select c_select, c_unit, c_client_name, c_prev_reading, c_curr_reading, c_water_cost, c_garbage_fee, c_amt_due, c_remarks, c_pbl_id, c_entity_id, c_proj_id, c_seq_no from display_water_billing_units_v2('"+co_id+"', '"+proj_id+"','"+ph_no+"','"+dateReading.getDate().toString()+"')";
	...
	private void computeWaterCost() {
		...
		if(tblWBilling.getSelectedRowCount() == 1) {
			...
			if(isSelected) {
				...
				Double garbage_fee = null;
				String Curr_Rdng_str = null;
				BigDecimal current_reading = new BigDecimal("0.00");

				try { 
					Curr_Rdng_str = modelWBilling.getValueAt(selected_row, 4).toString().replace(",",""); 
				} catch (NullPointerException e) { 
					Curr_Rdng_str = "0"; 
				}
				...
				if(proj_id.equals("017")) {
					modelWBilling.setValueAt(BigDecimal.valueOf(Curr_Rdng*23.00), selected_row, 5);
				}
				...
				if(proj_id.equals("018")) {
					modelWBilling.setValueAt(BigDecimal.valueOf(Curr_Rdng*39.00), selected_row, 5);
				}
		...
		for(int x= 0; x< modelWBilling.getRowCount(); x++) {
			...
			if(isSelected) {
				...
				if(proj_id.equals("017")) {
					Totl_Rdng = Curr_Rdng - Prev_Rdng;
					garbage_fee = 0.00;
					modelWBilling.setValueAt(BigDecimal.valueOf(Totl_Rdng*23.00), x, 5);
	...
	private void previewAllBillings() {
		...
		//System.out.print
		String SQL = "select * from view_water_bill_v2('"+lookupCompany.getValue()+"', '"+ph_no+"', '', '"+proj_id+"', '', '', '"+dateReading.getDate()+"')";
