...
public class TagMoveOutAccounts extends _JInternalFrame implements _GUI, ActionListener,MouseListener {
...
//	JPanel pnlEast;
	...
	static String title = "Tag Move Out Accounts";
	Dimension frameSize = new Dimension(700, 500);// 510, 250
	Border lineBorder = BorderFactory.createLineBorder(Color.GRAY);

	public TagMoveOutAccounts() {
		super(title, false, true, false, true);
		initGUI();
	}

	public TagMoveOutAccounts(String title) {
		super(title, false, true, false, true);
		initGUI();
	}

	public TagMoveOutAccounts(String title, boolean resizable,
				...
				}
				{
					...
					btnCancel.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							int response = JOptionPane.showConfirmDialog(TagMoveOutAccounts.this.getTopLevelAncestor(),"Are you sure you want to Clear all fields?   ",
	...
	private void displayTTOAccounts(String proj, String phase, String block) {
		...
		String sql = "select * from view_move_out_v2('"+proj+"', '"+phase+"', '"+block+"')";
		...
		if(actionCommand.equals("Save")){
			...
			for(int x =0; x < FullSettled.getRowCount(); x++){
				...
				if(isSelected){
					...
					strSQL = "SELECT sp_save_tag_move_out('"+entity+"', "+seq+", '"+to_date+"'::timestamp, '"+lookupProject.getValue()+"', '"+pbl+"', '"+lookupCompany.getValue()+"',  '"+UserInfo.EmployeeCode +"');" ; 
