...
public class TagRMCnumberTcost extends _JInternalFrame {
	...
	static String title = "Tag Request MC number TCOST - Disbursement Voucher";
	...
	private static _JLookup lookupTcostID;
	...
	public TagRMCnumberTcost(){
		super(title,true,true,false,false);
		initGui();
	}
	public TagRMCnumberTcost(String title){
		super(title);
		initGui();
	}
	public TagRMCnumberTcost(String title,boolean resizable,boolean closable,boolean maximizable,boolean iconifiable){
...

			{
				...
				}
				{
					...
					mainNorth.add(mainNorthComponents,BorderLayout.CENTER);
					{
							...
							lookupCompany.addLookupListener(new LookupListener() {
								public void lookupPerformed(LookupEvent event) {
									...
									if(data != null){
										...
										lookupTcostID.setEnabled(true);
										lookupTcostID.setLookupSQL(getTcostid(lookupCompany.getValue()));
						...
						{
							lookupTcostID = new _JLookup(null, "TCOST ID");
							mainNorthComponents.add(lookupTcostID);
							lookupTcostID.setReturnColumn(0);
							lookupTcostID.addLookupListener(new LookupListener() {
								public void lookupPerformed(LookupEvent event) {
									Object[] data = ((_JLookup)event.getSource()).getDataSet();
									if(data != null){
	...
	public static String getTcostid(String co_id) {
		return "	SELECT distinct on (tcostdtl_id::int) tcostdtl_id, trim(tcostdtl_desc) as tcost_desc, tcostdtl_amt, remarks\n" + 
				...
				"									WHERE status_id = 'A'\n" + 
				"                                   -- and tcostid_dl in ('008','011','012','021','169')\n" + 
				"                                    and server_id is null\n" + 
				"\n" + 
				"								)\n" + 
				"					order by tcostdtl_id::int ASC";
		...
		SwingWorker sw = new SwingWorker() {
			protected Object doInBackground()
				...
				String sql = 
						...
						"select * from view_TagRMCnumberTcost('"+ lookupTcostID.getValue() +"','"+ lookupCompany.getValue() +"')";
	...
	private void preview() {
		
		{
			...
			mapParameters.put("tcost_id", lookupTcostID.getValue());
			...
			FncReport.generateReport("/Reports/rptTagRMCTcost.jasper", "F", txtCompany.getText(),mapParameters);
			...
			if (JOptionPane.showConfirmDialog(getContentPane(), "Do you want to save entries?", "Save", JOptionPane.YES_NO_OPTION,
				...
				for (int x = 0; x < modelRMC_cv.getRowCount(); x++) {
					...
					if(isSelected) {
						...
						String sql = "INSERT INTO public.rf_rmctcost_detail(\n" + 
