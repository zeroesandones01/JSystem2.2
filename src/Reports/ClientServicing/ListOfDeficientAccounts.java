...
public class ListOfDeficientAccounts extends _JInternalFrame implements _GUI, ActionListener {
	static String title = "OR Accounts Not yet Docs Complete";
	...
	public ListOfDeficientAccounts() {
		super(title, false, true, false, true);
		initGUI();
	}

	public ListOfDeficientAccounts(String title) {
		super(title, false, true, false, true);
		initGUI();
	}

	public ListOfDeficientAccounts(String title, boolean resizable, boolean closable,
							...
							lookupPhase.addLookupListener(new LookupListener() {
								...
								public void lookupPerformed(LookupEvent event) {
									...
									if(data != null){
										...
										String phase_desc = (String) data[2];
	...
	public void actionPerformed(ActionEvent e) { 
		...
		if(actionCommand.equals("Preview")){
			...
			FncReport.generateReport("/Reports/rptPagibigStatusMonitoring_PQA_DeficientAccts.jasper", "Deficient Accounts", mapParameters);
		}
		
		if(actionCommand.equals("Export")){
			String col_names [] = {"Div.", "Dept.", "Client", "Unit", "Pmt Stage" ,"OR Date","House %","NTP Date","MSVS Approved", "MSVS Awaiting Release", "MSVS w/ Findings", "SCD Out", "SCD In", "ESAV", "Payslip", "BSL",
					"Pmt Status", "1st Date Prepared", "1st Received Date", "1st Relation", "1st Date Sent" ,"1st Received By", "Final Date Prepared", "Final Received Date", 
					"Final Relation", "Final Date Sent" ,"Final Received By"};

			String SQL = "SELECT * FROM view_docs_completion_deficient_accts_export(NULLIF('"+lookupCompany.getValue()+"', 'null'), NULLIF('"+lookupProject.getValue()+"', 'null'), NULLIF('"+lookupPhase.getValue()+"', 'null'));";
