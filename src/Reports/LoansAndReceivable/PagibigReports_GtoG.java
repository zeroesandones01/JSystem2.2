...
public class PagibigReports_GtoG extends _JInternalFrame implements ActionListener, _GUI {

	private static final long serialVersionUID = 4824765883993218355L;
	static String title = "REM G TO G FACILITY REPORT";
	...
	public PagibigReports_GtoG() {
		super(title, false, true, true, true);
		initGUI(); 
	}

	public PagibigReports_GtoG(String title) {
		super(title);
		initGUI();
	}
				...
				}
				{
					...
					panReport.setBorder(JTBorderFactory.createTitleBorder("Report Name", FncLookAndFeel.systemFont_Bold.deriveFont(10f)));
					{
						...
						bagThree.insets = new Insets(5, 3, 5, 3);
						{
							...
							db.select("select other_report_id::varchar || ' - ' || report_name from mf_hdmf_other_reports_gtog where other_report_id in (1,2,3) order by other_report_id");
	...
	private void Generate(String strReport) {
		...
		if (strReport.equals("2 - REM Conversion Movement Report (G to G)")) { 
			...
			FncReport.generateReport("/Reports/rpt_hdmf_rem_conversion_status_report_gtog_new.jasper", strReport, "", mapParameters);

		} else if (strReport.equals("3 - Loan Released Not Endorsed for REM (G to G)")) {
			...
			FncReport.generateReport("/Reports/rpt_hdmf_loanReleasedNotEndorsedForREM_gtog.jasper", strReport, "", mapParameters);
		
		}/* else if (strReport.equals("4 - Cancelled Accounts (G to G)")) {
			mapParameters.put("01_Company", txtCompany.getText());
			mapParameters.put("02_AsOfDate", strDate);
			mapParameters.put("03_CoID", txtCoID.getText());
			mapParameters.put("04_ProID", strProject);
			...
			FncReport.generateReport("/Reports/rpt_hdmf_rem_cancelled.jasper", strReport, "", mapParameters);
		
		} else if (strReport.equals("5 - Full Settled Accounts (G to G)")) {
			mapParameters.put("01_Company", txtCompany.getText());
			mapParameters.put("02_AsOfDate", strDate);
			mapParameters.put("03_CoID", txtCoID.getText());
			mapParameters.put("04_ProID", strProject);
		...
		} else if (strReport.equals("6 - Officially REM Accounts (G to G)")) {
			mapParameters.put("01_Company", txtCompany.getText());
			mapParameters.put("02_AsOfDate", strDate);
			mapParameters.put("03_CoID", txtCoID.getText());
			mapParameters.put("04_ProID", strProject);
