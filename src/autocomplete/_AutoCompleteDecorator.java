 ...
 */
public class _AutoCompleteDecorator {
	...
	public static void decorate(final JComboBox comboBox, final ObjectToStringConverter stringConverter) {
		...
		final AutoCompleteDocument document = createAutoCompleteDocument(adaptor, strictMatching, stringConverter, editorComponent.getDocument());
		...
		final KeyListener keyListener = new AutoCompleteKeyAdapter() {
			public void keyPressed(KeyEvent keyEvent) {
				...
				if (comboBox.isDisplayable() && !comboBox.isPopupVisible()) {
					...
					for(int x=0; x < model.getSize(); x++){//XXX
						//System.out.println(model.getElementAt(x));
					}
	...
	static class NonStrictBackspaceAction extends TextAction {
		private static final long serialVersionUID = -5965849553352813524L;
	...
	static Object errorFeedbackAction = new TextAction("provide-error-feedback") {
		private static final long serialVersionUID = -6389793176906614566L;
