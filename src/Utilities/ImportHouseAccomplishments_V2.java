...
public class ImportHouseAccomplishments_V2 extends _JInternalFrame implements _GUI {
	...
	public ImportHouseAccomplishments_V2() {
		super(title, false, true, false, true);
		initGUI();
	}

	public ImportHouseAccomplishments_V2(String title) {
		super(title, false, true, false, true);
		initGUI();
	}

	public ImportHouseAccomplishments_V2(String title, boolean resizable,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(actionCommand.equals("Import")){
			...
			for(int x=0; x < modelAddUnit.getRowCount(); x++){
				...
				//System.out.printf("Display value of unit: %s%n", unit);
				
				if(isSelected && unit.trim().equals("") == false){
				
				String phase = String.valueOf(modelAddUnit.getValueAt(x, 2));//(String) modelAddUnit.getValueAt(x, 2);																																																																																																																																																																																																																																																																																																																																														
				String block = String.valueOf(modelAddUnit.getValueAt(x, 3));//(String) modelAddUnit.getValueAt(x, 3);																																				
				String lot = String.valueOf(modelAddUnit.getValueAt(x, 4));//(String) modelAddUnit.getValueAt(x, 4);
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																											
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																										Date date_from = (Date) modelAddUnit.getValueAt(x, 10);
				Date date_to = (Date) modelAddUnit.getValueAt(																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																											x, 11);
				BigDecimal percent = (BigDecimal) modelAddUnit.getValueAt(x, 15);
				String remarks = ((String) modelAddUnit.getValueAt(x, 22)).trim().replace("'", "''");
				String contractor = String.valueOf(modelAddUnit.getValueAt(x, 7)).replace("'", "''");
				String contract_no = (String) modelAddUnit.getValueAt(x, 8);
				String insp = (String) modelAddUnit.getValueAt(x, 9);

				
					//System.out.println("Dumaan dito sa selected");
					System.out.println("SELECT sp_save_house_accomplishment('"+unit+"', '"+phase+"'::varchar, '"+block+"'::varchar, '"+lot+"'::varchar, 'New Unit',"
								+ "'"+UserInfo.EmployeeCode+"', now()::timestamp, '"+dateTo.getDate()+"'::timestamp, now()::timestamp,"
								+ "'"+date_from+"'::TIMESTAMP, '"+date_to+"'::TIMESTAMP, "+percent+"::numeric, '"+remarks+"'::VARCHAR, '"+contractor+"'::VARCHAR, '"+contract_no+"', '"+insp+"','"+proj_id+"')");
			...
			JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Accomplishment succesfully uploaded.", "Save", JOptionPane.INFORMATION_MESSAGE);
...
/*		
	...
	private static String batch() {
		...
		String SQL = "SELECT to_char(COALESCE(MAX(batch_no::INT), 0), 'FM0000000000') FROM rf_unit_status WHERE status_id = 'A' and batch_no is not null and batch_no != ''" ;
	...
	private  static  Date setHouseAccomp() {
		pgSelect db = new pgSelect();
		
		String query = "SELECT set_house_accomp_date()";
		db.select(query);
