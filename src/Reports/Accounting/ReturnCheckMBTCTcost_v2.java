...
public class ReturnCheckMBTCTcost_v2 extends _JInternalFrame {
	...
	private JLabel lblCompany,lblProject,lblPhase,lblRplf;
	private JButton btnPreview;
	private JTextField txtCompany, txtProject, txtPhase, txtRplf;

	private _JLookup lookupCompany, lookupProject, lookupPhase, lookupRplf;
	...
	public ReturnCheckMBTCTcost_v2(){
		super(title,true,true,false,false);
		initGui();
	}
	public ReturnCheckMBTCTcost_v2(String title){
		super(title);
		initGui();
	}
	public ReturnCheckMBTCTcost_v2(String title,boolean resizable,boolean closable,boolean maximizable,boolean iconifiable){
...
//						lblRplf = new JLabel("Rplf No.:");
//						lblMainNorth.add(lblRplf);
					}
				}
				{
...
//							lookupRplf = new _JLookup(null, "Rplf");
//							mainNorthComponents.add(lookupRplf);
...
//				{
//					txtRplf = new JTextField();
//					mainNorthComponents2.add(txtRplf);
//					txtRplf.setEditable(false);
//					txtRplf.setPreferredSize(new Dimension(350,0));
//				}
...
//	}
	...
	private void displayPCOST(final DefaultTableModel modelPCOSTList, final String co_id, final String proj_id, final String phase) {
		FncTables.clearTable(modelPCOSTList);
		SwingWorker sw = new SwingWorker() {
			protected Object doInBackground()
					throws FileNotFoundException, IOException, InterruptedException {
				FncGlobal.startProgress("Loading...");
				modelPCOSTList.setRowCount(0);
				DefaultListModel listmod = new DefaultListModel();
				rowHeadPcost.setModel(listmod);

				String sql = 
						"select distinct on (a.retrn_check_no)\n" + 
								"false,\n" +
								"a.rplf_no as \"Rplf No.\", \n" + 
								"get_employee_name(a.created_by) as \"Created By\",\n"+
						...
						"group by a.rplf_no, a.created_by, a.retrn_check_no, a.date_edited\n" + 
						"order by a.retrn_check_no asc;"  ;
				...
				if(db.isNotNull()) {
					for(int x=0; x < db.getRowCount(); x++){
						modelPCOSTList.addRow(db.getResult()[x]);
...
//
//		if(tblPCOST.getSelectedRows().length==1);{
//			int row = tblPCOST.getSelectedRow();
//			String Rplf = (String) modelPCOSTList.getValueAt(row, 1);
//			String Requested = (String) modelPCOSTList.getValueAt(row, 2);
//			Timestamp Datec = (Timestamp) modelPCOSTList.getValueAt(row , 3);
//			String  Returned = (String) modelPCOSTList.getValueAt(row, 4);
//
//
//		}
//	}
	
	private void previewRPLF() {
		
		ArrayList<String> listRPLF = new ArrayList<String>();
		
		for(int x=0; x<modelPCOSTList.getRowCount(); x++) {
			Boolean isSelected = (Boolean) modelPCOSTList.getValueAt(x, 0);
			if(isSelected) {
				String rplf_no = (String) modelPCOSTList.getValueAt(x, 1);
				listRPLF.add(rplf_no);
			}
		}
		
		String rplf_no = listRPLF.toString().replaceAll("\\[|\\]", "").replaceAll(" ", "");
		...
		FncReport.generateReport("/Reports/rptReturnCheckForTcost.jasper", title,txtCompany.getText(), txtProject.getText(), mapParameters);
		System.out.println("/Reports/rptReturnCheckForTcost.jasper");
		System.out.println("SELECT * FROM view_returnMBTCTcost('"+lookupCompany.getValue()+"','"+txtCompany.getText()+"','"+lookupProject.getValue()+"','"+lookupPhase.getValue()+"','"+rplf_no+"')");
	}
