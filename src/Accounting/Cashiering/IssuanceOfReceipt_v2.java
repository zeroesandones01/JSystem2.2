 ...
 *
 */
public class IssuanceOfReceipt_v2 extends _JInternalFrame implements ActionListener, _GUI, MouseListener {
	...
	private JButton btnAddCashReturn;
	private JButton btnRemoveCashReturn;
	
	private JSplitPane splitPaymentList;
	private _JScrollPaneMain scrollCashReturnList;
	private _JTableMain tblCashReturnList;
	private JList rowHeaderCashReturnList;
	private _JScrollPaneTotal scrollCashReturnListTotal;
	private _JTableTotal tblCashReturnTotal;
	private modelIssuanceCashReturn modelCashReturnTotal;
	private modelIssuanceCashReturn modelCashReturn;
	...
	public IssuanceOfReceipt_v2() {
		super(title, true, true, true, true);
		initGUI();
	}

	public IssuanceOfReceipt_v2(String title) {
		super(title);
		initGUI();
	}

	public IssuanceOfReceipt_v2(String title, boolean resizable, boolean closable, boolean maximizable,
			boolean iconifiable) {
...
//					pnlCenterTable.setBorder(components.JTBorderFactory.createTitleBorder("Payment List"));
					...
					{
						splitPaymentList = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
						pnlCenter.add(splitPaymentList, BorderLayout.CENTER);
						splitPaymentList.setOneTouchExpandable(true);
						
						{
							JPanel pnlPaymentList = new JPanel(new BorderLayout(3, 3));
							splitPaymentList.add(pnlPaymentList, JSplitPane.LEFT);
							pnlPaymentList.setBorder(components.JTBorderFactory.createTitleBorder("Payment List"));
							{
						...
						{
							JPanel pnlCashReturnList = new JPanel(new BorderLayout(3, 3));
							splitPaymentList.add(pnlCashReturnList, JSplitPane.RIGHT);
							pnlCashReturnList.setBorder(components.JTBorderFactory.createTitleBorder("Cash Return List"));
							{
								scrollCashReturnList = new _JScrollPaneMain();
								pnlCashReturnList.add(scrollCashReturnList, BorderLayout.CENTER);
								scrollCashReturnList.addMouseListener(new MouseAdapter() {
									public void mouseClicked(MouseEvent e) {
										tblCashReturnList.clearSelection();
										try {
											tblCashReturnList.getCellEditor().stopCellEditing();
										} catch (NullPointerException e1) {
										}
									}
								});
								{
									modelCashReturn = new modelIssuanceCashReturn();
									modelCashReturn.addTableModelListener(new TableModelListener() {
										public void tableChanged(TableModelEvent e) {
											if (e.getType() == TableModelEvent.INSERT) {
												((DefaultListModel) rowHeaderCashReturnList.getModel()).addElement(modelCashReturn.getRowCount());
												scrollCashReturnListTotal.setRowHeaderView(FncTables.getRowHeader_Footer(Integer.toString(modelCashReturn.getRowCount())));
											}
											if (e.getType() == TableModelEvent.DELETE) {
												if (modelCashReturn.getRowCount() == 0) {
													rowHeaderCashReturnList.setModel(new DefaultListModel());
													scrollCashReturnListTotal.setRowHeaderView(FncTables.getRowHeader_Footer(
															Integer.toString(modelCashReturn.getRowCount())));
													// adjustRowModel();
												}
											}

											try {
												BigDecimal totalAmount = _IssuanceOfReceipt.totalCashReturn(modelCashReturn,
														modelCashReturnTotal);
												txtTotalAmount.setValue(totalAmount);
											} catch (ArrayIndexOutOfBoundsException e1) {
											}
										}
									...
									tblCashReturnList = new _JTableMain(modelCashReturn);// XXX Payment List
									scrollCashReturnList.setViewportView(tblCashReturnList);
									tblCashReturnList.addMouseListener(new MouseAdapter() {
										public void mousePressed(MouseEvent e) {
											if (tblCashReturnList.rowAtPoint(e.getPoint()) == -1) {
												tblCashReturnTotal.clearSelection();
											} else {
												tblCashReturnList.setCellSelectionEnabled(true);
											}
										}
										...
										public void mouseClicked(MouseEvent evt) {
											if ((evt.getClickCount() >= 2)) {
												displayParticularsCashReturn();
											}
										}
									});

									tblCashReturnList.addKeyListener(new KeyAdapter() {
										public void keyReleased(KeyEvent arg0) {
											if (arg0.getKeyCode() == KeyEvent.VK_F2) {
												displayParticularsCashReturn();
											}
									...
									tblCashReturnList.getColumnModel().getColumn(4).setCellRenderer(TextRenderer
											.getTextRenderer(SwingConstants.CENTER, "Double-click to select payment type."));
									;

									tblCashReturnList.addPropertyChangeListener("tableCellEditor", new PropertyChangeListener() {
										public void propertyChange(PropertyChangeEvent arg0) {
											if (arg0.getOldValue() != null) {
												tblPaymentList.packAll();
											}
									...
									tblCashReturnList.addMouseListener(this);
								}
								{
									rowHeaderCashReturnList = tblCashReturnList.getRowHeader();
									rowHeaderCashReturnList.setModel(new DefaultListModel());
									scrollCashReturnList.setRowHeaderView(rowHeaderCashReturnList);
									scrollCashReturnList.setCorner(ScrollPaneConstants.UPPER_LEFT_CORNER, FncTables.getRowHeader_Header());
								}
							}
							{
								scrollCashReturnListTotal = new _JScrollPaneTotal(scrollCashReturnList);
								pnlCashReturnList.add(scrollCashReturnListTotal, BorderLayout.SOUTH);
								{
									modelCashReturnTotal = new modelIssuanceCashReturn();
									modelCashReturnTotal.addRow(new Object[] { null, "Total", 0.00, null, null, null });

									tblCashReturnTotal = new _JTableTotal(modelCashReturnTotal, tblCashReturnList);
									scrollCashReturnListTotal.setViewportView(tblCashReturnTotal);

									tblCashReturnTotal.setTotalLabel(1);
								}
								scrollCashReturnListTotal.setCorner(ScrollPaneConstants.LOWER_LEFT_CORNER,
										displayTableNavigationCashReturn());
							}
						}
					}
					splitPaymentList.getLeftComponent().setVisible(true);
					splitPaymentList.getRightComponent().setVisible(false);
				}
		...
		tblCashReturnList.hideColumns("Part ID", "Check Type ID", "Bank ID", "Branch ID", "Receipt ID");
	...
	private void newCashReturn() {
		...
		splitPaymentList.getLeftComponent().setVisible(false);
		splitPaymentList.getRightComponent().setVisible(true);
		modelCashReturn.setEditable(true);
		...
		btnAddCashReturn.setEnabled(true);
		btnRemoveCashReturn.setEnabled(true);
	...
	private JPanel displayTableNavigationCashReturn() {
		
		btnAddCashReturn = new JButton(new ImageIcon(this.getClass().getClassLoader().getResource("Images/Science-Plus2-Math-icon.png")));
		btnAddCashReturn.setActionCommand("Add Row Cash Return");
		btnAddCashReturn.setToolTipText("Add Row");
		btnAddCashReturn.setEnabled(false);
		btnAddCashReturn.addActionListener(this);
		
		btnRemoveCashReturn = new JButton(new ImageIcon(this.getClass().getClassLoader().getResource("Images/Science-Minus2-Math-icon.png")));
		btnRemoveCashReturn.setActionCommand("Remove Row Cash Return");
		btnRemoveCashReturn.setToolTipText("Remove Row");
		btnRemoveCashReturn.setEnabled(false);
		btnRemoveCashReturn.addActionListener(this);
		
		JPanel pnl = new JPanel(new GridLayout(1, 2));
		pnl.add(btnAddCashReturn);
		pnl.add(btnRemoveCashReturn);
		...
		if (column == 1 && modelPaymentList.isEditable()) {
			...
			if (selectedTransaction.equals("CASH RETURN")) {
				SQL = "SELECT TRIM(pay_part_id)::VARCHAR as \"ID\", TRIM(partdesc) as \"Name\", TRIM(particulars) as \"Alias\" "
						+ "FROM mf_pay_particular WHERE (group_id = '06' OR pay_part_id IN ('179', '204', '214', '212', '197')) AND status_id = 'A' ORDER BY partdesc;";
			} else {
				if (entity_id == null) {
	...
	private void displayParticularsCashReturn() {// XXX
		String selectedTransaction = lblTransactionType.getText().replaceAll("\\[|\\]", "").trim();
		String entity_id = lookupClient.getValue();
		String proj_id = lookupProject.getValue();
		String pbl_id = txtUnitID.getText();
		...
		int column = tblCashReturnList.convertColumnIndexToModel(tblCashReturnList.getSelectedColumn());
		int row = tblCashReturnList.getSelectedRow();

		if (column == 2 && modelCashReturn.isEditable()) {
			String SQL = "";
			if (selectedTransaction.equals("CASH RETURN")) {
				SQL = "SELECT TRIM(pay_part_id)::VARCHAR as \"ID\", TRIM(partdesc) as \"Name\", TRIM(particulars) as \"Alias\" "
						+ "FROM mf_pay_particular WHERE (group_id = '06' OR pay_part_id IN ('179', '204', '214', '212', '197')) AND status_id = 'A' ORDER BY partdesc;";
			...
			Object[] data = dlg.getReturnDataSet();
			if (data != null) {
				modelCashReturn.setValueAt(data[0], row, 1);
				modelCashReturn.setValueAt(data[1], row, column);
				tblCashReturnList.packAll();
			}
		}

		if (column == 4 && modelCashReturn.isEditable()) {
			Object[] option = new Object[] { "Cash", "Check" };
			int payment_type = JOptionPane.showOptionDialog(this.getTopLevelAncestor(), "Please select payment type.",
					"Payment Type", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, option, 0);
			modelCashReturn.setValueAt(option[payment_type], row, column);
			tblCashReturnList.packAll();
		}

		if (column == 7 && modelCashReturn.isEditable()) { // lookup for Check Type
			_JLookupTable dlg = new _JLookupTable(FncGlobal.homeMDI, null, "Check", _OrderOfPayment.getCheckType(),
					false);
			dlg.setLocationRelativeTo(FncGlobal.homeMDI);
			dlg.setVisible(true);
			...
			if (data != null) {
				// modelPaymentList.setValueAt("Check", row, 3);
				modelCashReturn.setValueAt(data[0], row, 6);
				modelCashReturn.setValueAt(data[1], row, column);
				tblPaymentList.packAll();
			}
		}

		if (column == 11 && modelCashReturn.isEditable()) { // lookup for Bank
			_JLookupTable dlg = new _JLookupTable(FncGlobal.homeMDI, null, "Bank", _OrderOfPayment.getBank(), false);
			dlg.setLocationRelativeTo(FncGlobal.homeMDI);
			dlg.setVisible(true);
			...
			if (data != null) {
				// modelPaymentList.setValueAt("Check", row, 3);
				modelCashReturn.setValueAt(data[0], row, 10);
				modelCashReturn.setValueAt(data[1], row, column);
				tblCashReturnList.packAll();
			}
		}

		if (column == 13 && modelCashReturn.isEditable()) { // lookup for Bank Branch
			String bank_id = (String) modelCashReturn.getValueAt(row, 10);
			if (bank_id == null) {
				JOptionPane.showMessageDialog(this.getTopLevelAncestor(), "Please select bank first.", "Branch",
						JOptionPane.WARNING_MESSAGE);
				return;
			...
			if (data != null) {
				// modelPaymentList.setValueAt("Check", row, 3);
				modelCashReturn.setValueAt(data[0], row, 12);
				modelCashReturn.setValueAt(data[1], row, column);
				tblCashReturnList.packAll();
			}
		}
	...
	public void actionPerformed(ActionEvent e) {// XXX actionCommand
		...
		if (action.equals("Add Row Cash Return")) {
			addCashReturnRow();
		}
		
		if(action.equals("Remove Row Cash Return")) {
			removeCashReturn();
		}
	...
	private void cancel() {
		...
		modelCashReturn.clear();
		modelCashReturn.setEditable(false);
	...
	private void addCashReturnRow() {
		modelCashReturn.addRow(new Object[] {null, null, null, null, "Cash", null, null, null, null, null, null, null, null,
				null, null, null, null });
	}
	
	private void removeCashReturn() {
		int[] selectedRows = tblCashReturnList.getSelectedRows();
		...
		for (int x = selectedRows.length - 1; x > -1; x--) {
			...
			modelCashReturn.removeRow(row);
		}

		adjustRowModelCashReturn();
	}
			...
			if (toSave()) {
				if (JOptionPane.showConfirmDialog(this.getTopLevelAncestor(), "Are all entries correct?", "Save",
					...
					if (selectedTransaction.equals("HOLDING")) {
						...
						if (_IssuanceOfReceipt.saveCashReturn_v2(modelCashReturn, lookupClient.getValue(),
	...
	private Boolean checkTable() {
		...
		if(splitPaymentList.getRightComponent().isVisible()) {
			count = tblCashReturnList.getRowCount();
		}else {
			count = tblPaymentList.getRowCount();
		}
		...
		if(splitPaymentList.getRightComponent().isVisible()) {
			for (int x = 0; x < count; x++) {
				...
				try {
					particular = modelCashReturn.getValueAt(x, 2).toString();
				} catch (NullPointerException e) {
					particular = "";
				}
				try {
					amount = Double.parseDouble(modelCashReturn.getValueAt(x, 3).toString());
				} catch (NullPointerException e) {
					amount = 0.00;
				}
	...
	private void adjustRowModelCashReturn() {
		DefaultListModel listModel = new DefaultListModel();// Creating DefaultListModel for rowHeader.
		rowHeaderCashReturnList.setModel(listModel);
		Integer r = tblCashReturnList.getRowCount();
		Integer s = 1;
		while (s <= r) {
			listModel.addElement(s);
			s++;
		}
		adjustRowHeightCashReturn();
	}
	
	private void adjustRowHeightCashReturn() {// used

		int rw = tblCashReturnList.getModel().getRowCount();
		int x = 0;

		while (x < rw) {
			tblCashReturnList.setRowHeight(x, 20);
			x++;
		}
	}
