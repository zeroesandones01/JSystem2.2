...
public class pnlRB_RegularNotices_v2 extends JPanel implements ActionListener, _GUI {
	...
	public pnlRB_RegularNotices_v2(String SQL_PROJECT) {
		this.SQL_PROJECT = SQL_PROJECT;
		initGUI();
	}

	public pnlRB_RegularNotices_v2(LayoutManager layout) {
		super(layout);
		initGUI();
	}

	public pnlRB_RegularNotices_v2(boolean isDoubleBuffered) {
		super(isDoubleBuffered);
		initGUI();
	}

	public pnlRB_RegularNotices_v2(LayoutManager layout, boolean isDoubleBuffered) {
	...
	private void displayRegularNotices() {//XXX displayRegularNotices
		if(toGenerate()){
			String notice_id = ((String)cmbNoticeType.getSelectedItem()).split("-")[0].trim();

			rowHeaderNotices.setModel(new DefaultListModel());
			_RegularBillingAndNotices.displayRegularNotices(modelNotices, notice_id, lookupProject.getValue(), lookupPhase.getValue(), false, lookupBatchNo.getValue());
			scrollNotices.setCorner(ScrollPaneConstants.LOWER_LEFT_CORNER, FncTables.getRowHeader_Footer(Integer.toString(tblNotices.getRowCount())));
			tblNotices.packAll();
		}
	}
		...
		if(action.equals("Save")){
			...
			if(hasSelected()){
				if (JOptionPane.showConfirmDialog((JFrame) this.getTopLevelAncestor(), "Are all entries correct?", "Save", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
					...
					_RegularBillingAndNotices.saveRegularNotices(modelNotices, lookupProject.getValue(), notice_id, lookupBatchNo.getValue());
